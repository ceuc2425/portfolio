async function initV2() {
  try {
    ...
    const professionalLevels = ['Profesional', 'CFGS', 'Técnico', 'Bachillerato'];
    const techKeywords = ['programación', 'web', 'php', 'javascript', 'html', 'css', 'java', 'software', 'multiplataforma', 'informática', 'digital'];

    const mainEdu = data.education.filter(e => professionalLevels.includes(e.level));
    const techEdu = data.education.filter(e => 
      !professionalLevels.includes(e.level) && 
      (techKeywords.some(kw => e.title.toLowerCase().includes(kw)) || techKeywords.some(kw => e.institution.toLowerCase().includes(kw)))
    );
